...
func TestProcessor_generateOutputPath(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			outputDir: "",
			inputPath: filepath.Join("input", "image.jpg"),
			inputDir:  filepath.Join("input"),
			expected:  filepath.Join("input", "image_compressed.jpg"),
		},
		{
			name:      "出力ディレクトリ指定（相対パス維持）",
			outputDir: filepath.Join("output"),
			inputPath: filepath.Join("input", "subdir", "image.jpg"),
			inputDir:  filepath.Join("input"),
		...
		},
		{
			...
			outputDir: filepath.Join("output"),
			inputPath: filepath.Join("input", "image.png"),
			inputDir:  filepath.Join("input"),
