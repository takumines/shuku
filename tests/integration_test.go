...
func TestUnsupportedFormatIntegration(t *testing.T) {
	...
	// ダミーのWebPファイルを作成
	dummyWebpFile := filepath.Join(tempDir, "test.webp")
	err := os.WriteFile(dummyWebpFile, []byte("dummy webp content"), 0644)
	if err != nil {
		t.Fatalf("Failed to create dummy webp file: %v", err)
	}
	...
	// WebP圧縮コマンドを実行（失敗することを期待）
	cmd := exec.Command(binaryPath, "compress", "--input", dummyWebpFile, "--output", outputFile)
	output, err := cmd.CombinedOutput()
	...
	outputStr := string(output)
	expectedError := "サポートされていない画像形式です: .webp。現在はJPEG、PNG形式に対応しています。"
